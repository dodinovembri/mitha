                ...
                <div class="col-md-12">
                    <?php  
                        ...
                        foreach ($nilai as $nilai_key => $nilai_value) {
                            $psikotest = round($nilai_value * (25/100));
                            $query_nilai = "UPDATE `penyisihan` SET `psikotest` =  '$psikotest' WHERE `alternatif` = '$nilai_key' LIMIT 1 ;";
                            $result_nilai = mysqli_query($koneksi, $query_nilai);
                            $query_nilai = mysqli_query($koneksi, "UPDATE `peserta` SET `s_juri_psikotest` =  'sudah' WHERE `nim` = '$nilai_key' LIMIT 1 ;");                    
                            $query_2 = mysqli_query($koneksi, "UPDATE `penyisihan` SET `status_juri_psikotest` =  'sudah' WHERE `alternatif` = '$nilai_key' LIMIT 1 ;");                    
