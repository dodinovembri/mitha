              ...
              <div class="row">
                <div class="col-md-12">
                    <?php  
						...
						$test_tertulis_b = $_POST['tertulis'];
						$test_tertulis = round($test_tertulis_b * (60/100));

						
						$query_update = "UPDATE peserta SET `s_juri_tertulis`='sudah' WHERE nim='$nim'";
						$hasil_update = mysqli_query($koneksi, $query_update);
					
						$query_update_status_k2a = mysqli_query($koneksi, "UPDATE penyisihan SET `test_tertulis`='$test_tertulis', `status_juri_tertulis`='sudah' WHERE `alternatif`='$nim'");
						...
						// $query_update_status_k2a = mysqli_query($koneksi, "UPDATE penyisihan_tertulis SET `test_tertulis`='$test_tertulis', `status_juri_tertulis`='sudah' WHERE `alternatif`='$nim'");

						// $query_k1 = mysqli_query($koneksi, "SELECT * FROM penyisihan_tertulis where `alternatif`='$nim'");
						// while ($row = mysqli_fetch_array($query_k1)) {
						// 	$test_tertulis = $row['test_tertulis'];								
						// }
						// $query_k2 = mysqli_query($koneksi, "SELECT * FROM penyisihan_psikotest where `alternatif`='$nim'");
						// while ($row = mysqli_fetch_array($query_k1)) {							
						// 	$psikotest = $row['psikotest'];								
						// }
						// $query_k3 = mysqli_query($koneksi, "SELECT * FROM penyisihan_wii where `alternatif`='$nim'");
						// while ($row = mysqli_fetch_array($query_k1)) {							
						// 	$wii = $row['wii'];								
						// }
						
						// $total_k11 = ($test_tertulis + $psikotest + $wii)/3;
						// $total_k1 = $total_k11 * (60/100);
						// $insert_k1 = mysqli_query($koneksi, "UPDATE penyisihan SET `total`='$total_k1' WHERE `alternatif`='$nim'");
