...
<!-- akhir kepala surat -->
 <?php
                       ...
                       $sqlSelect = "SELECT nilai.*, alternatif.*, peserta.* FROM nilai JOIN alternatif ON nilai.no_peserta=alternatif.alternatif JOIN peserta ON alternatif.alternatif = peserta.nim WHERE nilai.no_peserta=alternatif.alternatif AND alternatif.jk='L' AND alternatif.tolak='no' ORDER BY rank ASC";	
                        $result = mysqli_query($koneksi, $sqlSelect); ?>

                        <table border='1' width='100%' style='margin-top: 10px;'>
                <h3 style="color: black">Pemenang Bujang</h3>
                            <thead>
                                <tr>                                    
                                    <th>Alternatif</th>                                   
                                    <th>Nama</th>                                                                
...
<!-- akhir kepala surat -->				
 <?php
                        include 'module/koneksi.php';
                       $sqlSelect = "SELECT nilai.*, alternatif.*, peserta.* FROM nilai JOIN alternatif ON nilai.no_peserta=alternatif.alternatif JOIN peserta ON alternatif.alternatif = peserta.nim WHERE nilai.no_peserta=alternatif.alternatif AND alternatif.jk='P' AND alternatif.tolak='no' ORDER BY rank ASC";
                        $result = mysqli_query($koneksi, $sqlSelect); ?>
              <table border='1' width='100%' style='margin-top: 10px;'>
                <h3 style="color: black">Pemenang Gadis</h3>
                            <thead>
                                <tr>                                    
                                    <th>Alternatif</th>                                   
                                    <th>Nama</th>                                                                
